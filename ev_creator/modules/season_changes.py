...
# Crystal Palace
    ...
    238: [0.0],    # Richards
    239: [0.0],    # Riedewald
    240: [0.0],    # Schlupp                    FIXXXXX INJURY
    241: [0.0],    # Tomkins                   FIXXXX INJURY
    242: [77.0],    # Ward
    243: [0.0],    # Whitworth
...
# Everton
    ...
    245: [74.0],    # Branthwaite
    246: [80.0],    # Calvert-Lewin
    247: [73.0],    # Coleman
    248: [0.0],    # Dele
    249: [85.0],    # A.Doucoure
    250: [70.0],    # Garner
...
# Luton
    ...
    316: [0.0],    # Bell               INJURED SHORT TERM
    317: [0.0],    # Burry
    318: [77.0],    # Burke
    319: [75.0],    # Campbell
...
# Newcastle
    ...
    417: [0.0],    # Krafth
    418: [0.0],    # Kuol
    419: [80.0],    # Lascelles         FIXXXX     
    420: [0.0],    # Lewis
    421: [64.0],    # S.Longstaff
    422: [0.0],    # Manquillo
...
def add_extra_players(master, players_raw):
    ...
    master_extra = pd.DataFrame({'code': [535818, 536694, 441302, 503714, 220362, 497894, 489639, 108796, 492831, 222683, 118342, 168090, 578614, 189776, 183751, 250735, 435973, 482609, 517052, 213198, 424876, 432422, 201440, 233821, 487053, 184254, 244954, 202641, 243557, 487702, 223434, 174310, 208904, 165183, 82738, 244262, 156700, 229164, 91046, 54738, 476369], 
                                ...
                                'fbref': [np.NaN]*n,
                                'understat': [11710, 12152, 11807.0, 9451.0, 6885.0, 11055.0, 11711.0, 11714.0, 11701.0, 6963.0, 7047.0, 205.0, 5563.0, 1343.0, 7383.0, 7790.0, 7498.0, 9017.0, 10048.0, 3300.0, 9788.0, 7958.0, 7746.0, 10386.0, 8831.0, 8858.0, 6221.0, 10913.0, 6556.0, 10620.0, 7943.0, 11718.0, 11715.0, 11713.0, 11722.0, 11723.0, 11717.0, 11719.0, 11721.0, 11712.0, 9619.0],
                                'transfermarkt': [np.NaN]*n,
                                '23-24': [122.0, 615.0, 609.0, 613.0, 611.0, 617.0, 152.0, 575.0, 594.0, 72.0, 101.0, 128.0, 156.0, 158.0, 159.0, 162.0, 168.0, 203.0, 211.0, 212.0, 309.0, 429.0, 466.0, 471.0, 519.0, 520.0, 584.0, 597.0, 599.0, 605.0, 606.0, 314.0, 315.0, 316.0, 317.0, 321.0, 326.0, 328.0, 340.0, 614.0, 602.0]})
